...
import javax.swing.*;
import src.modelo.procedencia.Procedencia;
import src.modelo.procedencia.ProcedenciaDAO;
import src.vista.VistaProcedencia;

import javax.swing.JOptionPane;
import javax.swing.table.DefaultTableModel;
import java.util.ArrayList;

public class ControladorProcedencia extends Controlador<Procedencia> {

	public VistaProcedencia vista = (VistaProcedencia) vistaSuper;
	public static ProcedenciaDAO procedenciaDAO = new ProcedenciaDAO();

	private int CarFlaAct = 0;
	private int action; // 1: agregar,

	// constructor de la clase, se pasan los parametros de la vistaEspecifica
	public ControladorProcedencia(VistaProcedencia v) {
		super(v, procedenciaDAO);
		this.vista = v;
		listar(vista.tabla);
	}
	...
	@Override
	public Procedencia getOficinaDetalle() {
		Procedencia classInterna = new Procedencia();
		String cod = vista.cod.getText();
		String pais = vista.pais.getText();
		String departamento = vista.departamento.getText();
		String provincia = vista.provincia.getText();
		String distrito = vista.distrito.getText();
		String estaRegis = vista.estaRegis.getText();

		// Creamos un objeto
		classInterna.setCod(Integer.parseInt(cod));
		classInterna.setPais(pais);
		classInterna.setDepartamento(departamento);
		classInterna.setProvincia(provincia);
		classInterna.setDistrito(distrito);
	...
	public void listar(JTable tabla) {
		...
		ArrayList<Procedencia> lista = procedenciaDAO.listar();
		Object[] objeto = new Object[6]; // cambiar segun el numero de atributos


		for (int i = 0; i < lista.size(); i++) {
			objeto[0] = lista.get(i).getCod();
			objeto[1] = lista.get(i).getPais();
			objeto[2] = lista.get(i).getDepartamento();
			objeto[3] = lista.get(i).getProvincia();
			objeto[4] = lista.get(i).getDistrito();
			objeto[5] = lista.get(i).getEstadoRegistro();
	...
	public void llenarDatosDeTablaSelecionada(int fila, String estadoRegistro) {
		...
		String pais = (String) vista.tabla.getValueAt(fila, 1).toString();
		String departamento = (String) vista.tabla.getValueAt(fila, 2).toString();
		String provincia = (String) vista.tabla.getValueAt(fila, 3).toString();
		String distrito = (String) vista.tabla.getValueAt(fila, 3).toString();
		String estareg = (String) vista.tabla.getValueAt(fila, 4).toString();

		vista.cod.setText(cod);
		vista.pais.setText(pais);
		vista.departamento.setText(departamento);
		vista.provincia.setText(provincia);
		vista.distrito.setText(distrito);
	...
	public void limpiar() {
		...
		vista.provincia.setText("");
		vista.distrito.setText("");
		vista.estaRegis.setText("");

		vista.cod.setEditable(true);
		vista.pais.setEditable(true);
		vista.departamento.setEditable(true);
		vista.provincia.setEditable(true);
		vista.distrito.setEditable(true);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == vista.adicionar) {
			...
			if (fila == -1) {
				...
				vista.cod.setEditable(false);
				vista.pais.setEditable(false);
				vista.departamento.setEditable(false);
				vista.provincia.setEditable(false);
				vista.distrito.setEditable(false);
				vista.estaRegis.setEditable(false);
